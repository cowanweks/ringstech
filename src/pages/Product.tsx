...
async function addToCart(product_id: string) {
	...
	fetch(`${API_URL}/cart?product_id=${product_id}&quantity=1&color=black`, {
		method: "POST"
	}).then(res => res.json())
...
function ProductInformation() {
	...
	return <section className="grid grid-cols-1 md:grid-cols-3 min-h-dvh bg-white">
		...
		<div className="flex flex-col gap-y-2 px-4 py-10">
			<p id="product_description" className="min-h-fit h-8">Samsung Galaxy A55</p>
			<p className="py-2">Brand: <span>Samsung</span></p>
			...
			<p className="flex items-center gap-x-2 py-2 text-red-500">
				<WarnIcon size={16} className="" /><span>{state.in_stock < 10 ? "3 units left" : ""}</span>
			</p>
