...
function ProductInformation() {
	...
	useEffect(() => {
		const fetchProduct = async () => {
			const response = await fetch(`${API_URL}/products?product_id=${product_id}`);
	...
	return (
		<section className="grid grid-cols-1 md:grid-cols-3 gap-8 min-h-dvh bg-white px-4 py-8 lg:px-16">
			<img
				src={product?.product_image ? `${API_URL}/images/?id=${product?.product_image}` : PlaceHolderPic}
			...
			<div className="flex flex-col gap-y-4 px-4 py-10">
				...
				<p className="text-xl text-red-500">{product && formatPrice(product.product_unit_price)}</p>
				<p className={`flex items-center gap-x-2 py-2 ${outOfStock ? "text-red-500" : "text-green-500"}`}>
					{outOfStock ? (
					...
					{product?.available_colors && product.available_colors.length > 0 ? (
						product.available_colors.map((color: string, index: number) => (
							<button
								...
								title=""
								key={index}
								className={`inline-block cursor-pointer h-12 w-12 rounded-full border-2 transition-transform ${selectedColor === color ? "border-gray-400 scale-105" : "border-transparent"
									}`}
						...
						<>
							...
							{product?.display && (
								<div className="flex gap-x-2 text-gray-500">
									Display: <span className="text-red-500">{product.display}</span>
								</div>
							)}
							{product?.battery && (
								<div className="flex gap-x-2 text-gray-500">
							...
							{product?.cameras && (
								<div className="flex gap-x-2 text-gray-500">
									Camera: <span className="text-red-500">{product.cameras}</span>
								</div>
							)}
						</>
					)}
